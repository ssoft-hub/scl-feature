...
void compileTestWrapperTrancpatancy()
{
    ...
    // по правилам C++ такой код не собирается для временных экземпляров (taking address of
    // temporary)
    //(*&TestType()).rvalueMethod();
    //(*&TestType()).rvalueConstMethod();
...
void testWrapperValue()
{
    ...
    {
        [[maybe_unused]]
        Wrapper<double> value{Wrapper<double>()};
    }
    {
        [[maybe_unused]]
        Wrapper<double> value = Wrapper<double>();
...
void testWrapperContainer()
{
    ...
    {
        [[maybe_unused]]
        Wrapper<Container> value{Container{}};
    }
    {
        [[maybe_unused]]
        Wrapper<Container> value = Container{};
