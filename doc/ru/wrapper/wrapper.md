...
- **Фундаментальные и классовые типы одинаково** — путь значения использует
  ...
  `double` и классы с оператором-членом `operator=` наравне. (В отличие от составных
  присваиваний `+=`, `*=`, …, которые отражаются через оператор-член значения и
  потому недоступны для фундаментальных типов.)
