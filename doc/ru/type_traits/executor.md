...
> `unguard()` обязаны допускать повторный (вложенный) захват: одно выражение
> может захватить блокировку одного исполнителя несколько раз (`w == w`,
> `a = a`, одна обёртка, переданная несколькими аргументами), поэтому голый
> нерекурсивный `std::mutex` приводит к самоблокировке — используйте счётчик или
> рекурсивную блокировку. Если для cv-ref-квалификации метода `unguard()` нет,
> `guard()` обязан быть самодостаточным.
